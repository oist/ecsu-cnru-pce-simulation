...
def get_num(genotype_structure, key):
    """
    TODO: Missing function docstring
    """
    assert key in genotype_structure, f"Invalid key: {key}"

    value = genotype_structure[key]
    return len(value['indexes']) \
        if 'indexes' in value \
        else len(value['default'])

def get_num_neurons(genotype_structure):
    return  get_num(genotype_structure, "neural_gains")
...
DEFAULT_GEN_STRUCTURE = lambda n: build_structure(
    num_sensors = 1,
    num_neurons = n,
    num_motors = 2
)
...
if __name__ == "__main__":
    default_gs = DEFAULT_GEN_STRUCTURE(3)
    check_genotype_structure(default_gs)
    print("Size: {}".format(get_genotype_size(default_gs)))
    print("Neurons: {}".format(get_num_neurons(default_gs)))
